...
from SVRG_NNAG import SVRG_NNAG , SVRG_Snapshot
from NNAG import NNAG
from utils import  CIFAR10_dataset, CIFAR10_ConvNet, AverageCalculator, accuracy, plot_train_stats

# The SVRG/SGD/SVRG_NNAG & the body implementation were inspired from https://github.com/yueqiw/OptML-SVRG-PyTorch

# For similar results as in the paper use SGD: lr 0.01 SVRG lr 0.01 NNAG lr 0.05 a 150 SVRG_NNAG lr 0.001 a 30 and all weight decays equal to 0
parser = argparse.ArgumentParser(description="Train SVRG/SGD/NNAG/NNAG_SVRG on CIFAR10 data.")
parser.add_argument('--optimizer', type=str, default="NNAG",
                    help="optimizer.")
...
parser.add_argument('--a', type=float, default=100,
                    help="tuning parameter (usually \sqrt{L}).")
...
def train_epoch_NNAG(model, optimizer, train_loader, loss_fn, flatten_img=True):
    model.train()
    loss = AverageCalculator()
    acc = AverageCalculator()
...
def train_epoch_SVRG_NNAG(model_k, model_snapshot, optimizer_k, optimizer_snapshot, train_loader, loss_fn, flatten_img=True):
    model_k.train()
    model_snapshot.train()
    loss = AverageCalculator()
    acc = AverageCalculator()
    ...
    for images, labels in train_loader:
        ...
        optimizer_snapshot.zero_grad()
        loss2.backward()
        optimizer_k.opt__init() # resetting the accumulating step size and the counter
        optimizer_k.step(optimizer_snapshot.get_param_groups())
...
if __name__ == "__main__":
    ...
    if not args.optimizer in ['SGD', 'NNAG', 'SVRG','NNAG_SVRG']:
        raise ValueError("--optimizer must be 'SGD' or 'NNAG' or 'SVRG' or 'NNAG_SVRG'.")
    print(args_dict)
    ...
    v_init = copy.deepcopy(model)
    v_init2 = copy.deepcopy(model)
    # the optimizer 
    if args.optimizer == "SGD":
        optimizer = SGD(model.parameters(), lr=lr, weight_decay=args.weight_decay)
    elif args.optimizer == "NNAG":
        optimizer = NNAG(params = model.parameters(), v=list(v_init.parameters()),lr=args.lr, t_k=0.0,s_k=0.0,a=args.a,weight_decay=args.weight_decay,k=0,alpha=3/4)
    ...
    elif args.optimizer == "NNAG_SVRG":
        optimizer = SVRG_NNAG(params =model.parameters(), v=list(v_init2.parameters()),lr=lr, weight_decay=args.weight_decay,t_k=0.0,s_k=0.0,a=args.a,k=0,alpha=3/4)
        optimizer_snapshot = SVRG_Snapshot(model_snapshot.parameters())

    # output folder 
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    model_name = timestamp + "_" + args.optimizer + "_" +  'CIFAR10_ConvNet'
    if args.exp_name != "":
    ...
    for epoch in range(n_epoch):
        ...
        if args.optimizer == "SGD":
            ...
            train_loss, train_acc = train_epoch_NNAG(model, optimizer, train_loader, loss_fn, flatten_img=flatten_img)
        elif args.optimizer == "SVRG":
            ...
            train_loss, train_acc = train_epoch_SVRG_NNAG(model, model_snapshot, optimizer, optimizer_snapshot, train_loader, loss_fn, flatten_img=flatten_img) 
        ...
        if (epoch + 1) % 5 == 0:
            ...
            plot_train_stats(train_loss_all, val_loss_all, train_acc_all, val_acc_all, log_dir, acc_low=0.0)
    # done
