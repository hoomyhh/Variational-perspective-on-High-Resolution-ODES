...
\newcommand{\remove}[1]{\textcolor{red}{[\textbf{Remove:} #1]}}
\newcommand{\replace}[2]{\textcolor{red}{[\textbf{Replace:} \sout{#1} \textcolor{blue}{#2}]}}
...
Smooth convex minimization is a fundamental subclass of optimization problems with broad applications %in various areas such as multivariate linear regression \citep{lu2012convex}, image processing \citep{iPiasco}, compressed sensing \citep{bobin2009fast}, and matrix completion \citep{zhang2011strongly}. This problem class has 
...
% \aynote{Should we also talk about discrete time interpretations? Like Linear coupling of Allen-Zhu and PPM perspective of Kwangjun?}
% \hmnote{I agree, how about continuing the last sentence as: ; \citep{Zhu2014LinearCA} utilizes a \textit{linear coupling} of gradient and mirror descent to recover the NAG method; and \citep{ahn2022understanding} provides an understanding of the NAG algorithm through an approximation of the \textit{proximal point method}.}
...
% \end{aligned}
% \end{equation}
% % with \(x_0=y_0\in\mathbb R^n\) and \(0< s\leq 1/L\).
...
% % \aynote{What is the difference between Wibisono and Wilson? Hoomaan: Wilson uses a different Lagrangian which fits the strongly convex functions. Here, I think it is better to remove Wilson's citation since it seems to be unrelated to mention that they did the same thing for different class of functions}  
...
In \Cref{section3}, we show that our proposed variational analysis yields a special representation of NAG leading to superior convergence rates than \citep{shi2019acceleration} in terms of gradient norm minimization. 
In \Cref{section4}, we propose a HR-ODE based on the rate-matching technique. We demonstrate that NAG can be interpreted as an approximation of the rate-matching technique applied to a specific ODE. 
Furthermore, in \Cref{section5}, we extend our analysis to a stochastic setting where gradients are noisy. The proposed stochastic method ensures convergence rates of $\tilde{\mathcal{O}}(1/k^{1/2})$ and $\tilde{\mathcal{O}}(1/k^{3/4})$ for the expected objective residual and the expected gradient norm, respectively. 
Finally, in \Cref{sec_numerical}, we present numerical experiments to demonstrate the empirical performance of the proposed method and to validate our theoretical findings. 

%\paragraph{Our contribution} We will positively respond to the first part of the question in \Cref{section2}. This is done by incorporating external forces while minimizing the action on the Lagrangian. We will show that same force with various damping parameters recovers various HR-ODEs. Our variational analysis leads to a special representation of NAG in \Cref{section3} which allows for better convergence rates for gradient norm minimization of NAG than \citep{shi2019acceleration}. In \Cref{section4}, we propose a HR-ODE of the rate-matching technique and show that the NAG method can be seen as an approximation of the rate-matching technique when applied on a special ODE. Finally, we extend our analysis to practical scenarios by considering noisy gradients in \Cref{section5}. Specifically, after \(k\) calls of the proposed method, we will show a convergence rate of \(\mathcal O(\log(k)/\sqrt{k})\) for \(\mathbb E[f(x_k)]-f^*\) and \(\mathcal{O}(\log(k)/k^{3/4})\) for \(\mathbb E\left[\min_{0\leq i\leq k-1}\|\nabla f(x_i)\|^2 \right]\). In \Cref{sec_numerical} a number of numerical simulations and comparison with state of the art methods illustrate our theoretical findings.


\paragraph{Problem template and notation.} % Consider the following unconstrained minimization problem
...
\paragraph{The NAG Algorithm.} Given an initial state $x_0 = y_0 \in \mathbb{R}^n$ and a step-size parameter $s > 0$, %  $s \in [0,1/L]$, 
...
\end{align}
We establish the following convergence result, and the proof can be found in \Cref{thm3_proof}.
...
\end{align}
 This ODE was discretized  using the Semi-Implicit Euler (SIE) and the Implicit Euler (IE) discretization schemes in \citep{shi2019acceleration}. The corresponding optimization algorithms were shown to accelerate. In addition, note that the convergence rate proved in \Cref{Theorem_ODE_Shi} is faster than its counterpart in \Cref{Theorem_ODE_laborde}.
 %Nevertheless, Shi et al., cannot guarantee convergence of their HR-ODE when discretized using either of the Semi-Implicit-Euler (SIE), Implicit Euler (IE), and EE numerical schemes (for more details see appendix C.2 in \citep{shi2019acceleration}). In addition, note that the convergence rate proved in \Cref{Theorem_ODE_Shi} is faster than its counterpart in \Cref{Theorem_ODE_laborde}.
\end{remark}
...
\end{align}
We can establish the following convergence result for \(X_t\) in (\ref{sc_eqn1}) to the unique minimizer \(x^*\). The proof of this result is deferred to \Cref{thm4_proof}.
...
One of the implications of our variational study on HR-ODEs in \Cref{section2} was the ODE (\ref{HR_cnts_general_stable_ODE}). Reformulating this ODE gives
...
\begin{remark}[Comparison with state of the art]
    The rate in \Cref{theorem4} is improved compared to the previous rate found in \citep{Shi2021UnderstandingTA}, which is 
...
%Indeed, rate-matching discretization can approximate the NAG method after discretizing a proper choice of ODE. We begin by showing that the rate-matching when applied to the LR-ODE 
...
%    \right.
%\end{align}
%Note that (\ref{cont_rate_match_2_perturb2}) is equivalent to (\ref{LR_cnts_form}) for \(p=2\). 
...
\begin{theorem}\label{Theorem5}
   Consider the conditions of \Cref{Theorem6}. Then, if the NNAG modifies to
...
\begin{remark}[Comparison between Theorems \ref{Theorem6} and \ref{Theorem5}]
    The rate in (\ref{Theorem7_rate2}) is asymptotically similar to (\ref{conv_rate1_them6}). However, the transient behaviour of (\ref{Theorem7_rate2}) is faster than both (\ref{conv_rate1_them6}) and the rate in \citep{pmlr-v108-laborde20a} when \(L\) is large (see \Cref{fig1}). This is due to the tuning parameter \(\beta\) which is usually set to \(L\) or higher. This scenario (Large \(L\)) often happens in practice, e.g. in training a two-layer Convolutional Neural Network (CNN) \citep{shi2022efficiently}. \par
...
%\begin{figure}[t]
%\centering
%\subfigure{\includegraphics[width=0.45\textwidth]{NIPS 2023/Data/upper bounds.eps}\label{fig1}\subcaption{(b)}}
...
\section{Appendix}
\subsection{Proof of \Cref{Theorem_ODE_laborde}}\label{thm1_proof}
...
\subsection{Proof of \Cref{Theorem_ODE_Shi}}\label{thm3_proof}
Consider the Lyapunov function 
...
and the proof is complete.
\subsection{Proof of \Cref{Theorem3_1}}\label{thm4_proof}
Consider the Lyapunov function
   ...
   & + e^{\beta_t}\langle \nabla f(X_t),\dot X_t \rangle.
    ...
    and the proof is complete.
\subsection{Proof of \Cref{theorem4}}\label{thm5_proof}
...
\subsection{Proof of \Cref{Theorem5}}\label{thm6_proof}
Take the Lyapunov function
    ...
    \begin{align}\label{Lyap_stc_4}
        ...
        \end{align}
        Here, the proof divides in 2 sections for each of the results in \Cref{Theorem5}. First, we prove the rate for $\mathbb E\left[f(x_k)\right]-f(x^*)$. 
...
\subsection{Proof of \Cref{Theorem6}}\label{thm7_proof}
Take the Lyapunov function
    ...
    \begin{align}\label{Lyap2_stc_4}
        ...
        \end{proof}
Next, using \Cref{lem1} in (\ref{Lyap2_stc_6}) gives 
