...
% \aynote{We need a \textbf{short} paragraph here, explaining the importance of this template.}
...
% \aynote{We mention the convergence rate of Gradient Descent. Then we should explain that the rate of Gradient Descent does not match the iteration complexity lower bounds, hence GD is not optimal.}

% \aynote{Then describe Nesterov's Accelerated Gradient Method as the first first-order method with optimal convergence rate.} 
...
\end{align}
 ...
 %Nevertheless, Shi et al., cannot guarantee convergence of their HR-ODE when discretized using either of the Semi-Implicit-Euler (SIE), Implicit Euler (IE), and EE numerical schemes (for more details see appendix C.2 in \citep{shi2019acceleration}). In addition, note that the convergence rate proved in Theorem \ref{Theorem_ODE_Shi} is faster than its counterpart in Theorem \ref{Theorem_ODE_laborde}.
\end{remark}
